...
//其他请求携带token
axios.defaults.headers.common['token'] = localStorage.getItem('h5token');
axios.interceptors.request.use(
    config => {
            config.headers.client = 'h5';
        if (localStorage.getItem('h5token')) {  // 判断是否存在token，如果存在的话，则每个http header都加上token
            config.headers.token = localStorage.getItem('h5token')
        }
        return config;
    },
    err => {
